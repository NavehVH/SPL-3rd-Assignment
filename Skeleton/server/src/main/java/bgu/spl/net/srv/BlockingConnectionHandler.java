    ...
    public void run() {
        try (Socket sock = this.sock) { // just for automatic closing
            ...
            while (!protocol.shouldTerminate() && connected && (read = in.read()) >= 0) {
                ...
                T nextMessage = encdec.decodeNextByte((byte) read);// the message send to the decoder byte after byte
                if (nextMessage != null) {
                    protocol.process(nextMessage);// after the encoding the message send to procces
